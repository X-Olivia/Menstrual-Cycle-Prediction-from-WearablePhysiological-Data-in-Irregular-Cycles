"""
Multi-Signal Ovulation Detection V2 — Comprehensive Algorithm Suite
====================================================================
Based on literature (Yu 2022, Grant 2020, Nature 2025):
  - HMM (2/3 state) for biphasic/triphasic phase detection
  - BOCPD (Bayesian Online Changepoint Detection)
  - CUSUM (Cumulative Sum) multi-signal changepoint
  - Phase-classification approach (follicular vs luteal boundary)
  - Wavelet denoising + gradient analysis
  - ARIMA residual anomaly detection
  - Enhanced ML with XGBoost/LightGBM + nocturnal HR/HRV features
  - Deep learning: 1D-CNN on multi-signal daily time-series
  - Stacking meta-learner: combines all methods

ALL methods are leakage-free:
  - No current cycle_len as feature
  - hist_cycle_len derived from PAST completed cycles only
  - LOSO for supervised methods
  - LH labels only for evaluation

Usage:
    cd /Users/xujing/FYP/main_workspace
    python -u -m model.experiment.run_multisignal_ov_v2
"""
...
def load_all_signals():
    """Load and aggregate ALL available signals (reuse from v1 with caching)."""
...
def main():
    print(f"\n{SEP}")
    print(f"  Multi-Signal Ovulation Detection V2 — Comprehensive Suite")
    print(f"{SEP}")
    t0 = time.time()
