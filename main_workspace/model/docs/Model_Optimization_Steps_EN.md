# Model Optimization Steps (Research-Grade Execution Order)
...
**If you skip this step, later optimisations may be wasted.** Run:
...
| Step | Action |
|------|--------|
| 3.1 | **Focal Loss** | Replace BCE with Focal Loss while keeping all samples and positive weighting. |
| 3.2 | **Binary labels (optional)** | Binarise soft labels (e.g. max-prob day = 1) for interpretability. |
...
| Step | Action |
|------|--------|
| 4.1 | **Always report ovulation_corr** | Val/Test ovulation_corr alongside MAE, acc_1d. |
| 4.2 | **Stratify by horizon; focus 6–20** | Optimise acc_1d for horizon 6–20 (luteal); report 21+ separately. |
| 4.3 | **Optional: ovulation day ±1 recall/precision** | Define ovulation day as max-prob day per cycle; report recall when predicted peak is within ±1 day of true. |
...
| Step | Action |
|------|--------|
| 5.1 | **Two-phase modelling** | First classify ovulation/already past ovulation; then predict menses from “days since ovulation” or luteal phase. |
| 5.2 | **Luteal length prior** | Add 12–16 day luteal prior in loss or output. |
