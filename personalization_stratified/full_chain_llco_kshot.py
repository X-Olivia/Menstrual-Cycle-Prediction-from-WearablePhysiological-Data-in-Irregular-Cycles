"""
Full-chain LLCO + k-shot (standalone).

Pipeline
--------
1. Ovulation day: LOSO multi-signal ML regression (`ml_detect_loso` from multisignal experiment),
   not oracle — detection noise flows into menses prediction.
2. Menses / remaining-days: same B1/B2/M3 + anchor protocol as `eval_llco_kshot.py`.
3. k-shot: still on the *calendar / bias* side (history cycle lengths for B2/M3). The detector is
   not few-shot adapted here (would require per-user fine-tuning); document as partial full-chain.
...
from detectors_ml import ml_detect_loso

from data_prep import build_subject_meta, load_core_data, make_subject_meta_df
from models_cyclelevel import ANCHORS_ALL, estimate_b1_history_bias, global_cycle_mean, predict_menses_start_b1, predict_menses_start_b2
...
def evaluate_full_chain_llco_kshot(
    lh_dict: Dict[str, int],
    cycle_series: Dict[str, dict],
...
def main() -> None:
    parser = argparse.ArgumentParser(description="LLCO + k-shot with LOSO ML ovulation (full chain)")
    ...
    parser.add_argument(
        ...
        type=str,
        default="ridge",
        help="Passed to ml_detect_loso (e.g. ridge, rf, gbdt)",
    )
    parser.add_argument(
        "--out-dir",
        type=str,
    ...
    print("[full_chain] Running ml_detect_loso (may take a minute)...")
    det_dict, _ = ml_detect_loso(cycle_series, lh_dict, model_type=args.model)
    print(f"[full_chain] Detections: {len(det_dict)} labeled cycles")

    long_df, agg_df = evaluate_full_chain_llco_kshot(
        lh_dict=lh_dict,
        cycle_series=cycle_series,
    ...
    if long_df.empty:
        print("No evaluable rows (check LOSO coverage / missing detections for test cycles).")
        return

    long_path = out_dir / "full_chain_llco_kshot_long.csv"
    agg_path = out_dir / "full_chain_llco_kshot_agg.csv"
    long_df.to_csv(long_path, index=False)
    agg_df.to_csv(agg_path, index=False)
    print(f"[DONE] long -> {long_path} ({len(long_df)} rows)")
    print(f"[DONE] agg  -> {agg_path} ({len(agg_df)} rows)")
    print(
